...
配置点击保存后才生效，会取消当前请求。API Key 按 Base URL 分别存入 macOS 钥匙串，不写入配置文件、对话或安装包。密钥框留空会保留**该地址**已存的密钥；勾选清除后保存可删除它。更换地址不会把旧地址的密钥自动转给新服务。

应用不再读取 `.secret` 或环境变量中的密钥，升级后请在 UI 中填写一次。旧的聊天、记忆、形象和观察设置继续保留。
...
测试覆盖 UI 配置默认值及旧数据迁移、钥匙串读写和地址隔离、自定义请求地址与思考参数、模型 JSON / 安静回复、本地数据重启恢复、损坏数据保护、权限和主题索引校验。独立测试程序无需完整 Xcode / XCTest；它不是命令行版小肥鱼。
