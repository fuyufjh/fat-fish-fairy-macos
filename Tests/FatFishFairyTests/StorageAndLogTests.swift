...
struct StorageAndLogTests {
    static func run() async throws {
        ...
        let emptyDiary = try store.diaryEntries()
        try SmokeTests.check(emptyDiary.isEmpty, "diary is empty before observations")
        try store.saveDailyMemo("10:30 review MR", for: "2026-09-22")
        try store.saveDailyMemo("09:00 写文档", for: "2026-09-23")
        try store.saveDailyMemo("10:30-15:50 review MR\n10:50-17:20 写文档", for: "2026-09-22")
        let diary = try StateStore(directory: root).diaryEntries()
        try SmokeTests.check(diary.map(\.day) == ["2026-09-23", "2026-09-22"], "diary survives restart in newest-first order without duplicate dates")
        try SmokeTests.check(diary.last?.content == "10:30-15:50 review MR\n10:50-17:20 写文档", "diary displays latest complete multiline memo")
